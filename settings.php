...
// Fetch existing PCs for dropdowns
...
.cards-wrapper{display:flex;gap:20px;justify-content:center;flex-wrap:wrap;width:100%;max-width:1100px;}
.card{background:rgba(10,25,47,.85);border:1px solid #132742;border-radius:12px;padding:26px;flex:1;min-width:400px;}
...
input,select{width:100%;padding:10px;background:#050814;border:1px solid #132742;color:white;border-radius:6px;box-sizing:border-box;}
...
.btn-add{background:#38bdf8;color:white;border:none;padding:0 18px;border-radius:6px;cursor:pointer;font-weight:bold;white-space:nowrap;}
...
.alert-success{background:rgba(46,204,113,.1);color:#2ecc71;border:1px solid #2ecc71;padding:12px;border-radius:8px;text-align:center;width:100%;max-width:1100px;}
        ...
        <div class="card">
            ...
            <form action="save_all_rates.php" method="POST">
                <div class="input-grid">
                    <div><label>1 HR Rate (₱)</label><input type="number" step="0.01" name="hour_rate"  value="<?= $rates['hourly_rate'] ?>"></div>
                    <div><label>3 HR Rate (₱)</label><input type="number" step="0.01" name="rate_3hr"   value="<?= $rates['rate_3hr'] ?>"></div>
                    <div><label>5 HR Rate (₱)</label><input type="number" step="0.01" name="rate_5hr"   value="<?= $rates['rate_5hr'] ?>"></div>
                    <div><label>7 HR Rate (₱)</label><input type="number" step="0.01" name="rate_7hr"   value="<?= $rates['rate_7hr'] ?>"></div>
                    <div><label>12 HR Rate (₱)</label><input type="number" step="0.01" name="rate_12hr" value="<?= $rates['rate_12hr'] ?>"></div>
                    <div><label>Min Charge (₱)</label><input type="number" step="0.01" name="min_charge" value="<?= $rates['minimum_charge'] ?>"></div>
                    <div><label>B&amp;W Print (₱)</label><input type="number" step="0.01" name="bw_rate"    value="<?= $rates['bw_rate'] ?>"></div>
                    <div><label>Color Print (₱)</label><input type="number" step="0.01" name="color_rate" value="<?= $rates['color_rate'] ?>"></div>
                </div>
